...
21. specify the exact default-off HLS/LL-HLS passive/view-only prototype contract without implementing a transport yet — **NEXT**,
22. keep accumulating reviewed implementation blocks on `testing`; promote to `master` only after the operator explicitly authorizes the final grouped promotion — **pending**.
