...
22. implement HLS/LL-HLS Phase 1: default-off configuration, authenticated bootstrap/playback-lease foundations, deterministic playlist/object models and golden fixtures without starting a packager or adding a client player — **NEXT**,
23. keep accumulating reviewed implementation blocks on `testing`; promote to `master` only after the operator explicitly authorizes the final grouped promotion — **pending**.
