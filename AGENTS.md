...
19. productize the still-explicit client choice without adding automatic fallback — **complete in the repository on `testing`; the per-client WebRTC/WebCodecs preference, URL override precedence and compact healthy status are implemented and statically reviewed, with target-server checks pending**,
20. validate the per-client media-backend productization on one exact `testing` commit while preserving the bounded Phase 4 limitations — **NEXT**,
21. keep accumulating reviewed implementation blocks on `testing`; promote to `master` only after the operator explicitly authorizes the final grouped promotion — **pending**.
