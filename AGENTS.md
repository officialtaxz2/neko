...
19. productize the still-explicit client choice without adding automatic fallback — **NEXT; persist a per-client WebRTC/WebCodecs preference in sidebar settings, retain the URL selector as a diagnostic override and replace the always-large healthy-stream overlay with a compact backend/status indicator**,
20. keep accumulating reviewed implementation blocks on `testing`; promote to `master` only after the operator explicitly authorizes the final grouped promotion — **pending**.
