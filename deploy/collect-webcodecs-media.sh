...
readonly COMPOSE_WEBCODECS="docker-compose.webcodecs-ws.yaml"
readonly RESULT_TEMPLATE="docs/WEBCODECS_MEDIA_WEBSOCKET_RESULTS_TEMPLATE.md"
readonly DEFAULT_METRICS_URL="http://127.0.0.1:8082/metrics"
readonly METRICS_PATTERN='^(go_goroutines|go_memstats_heap_alloc_bytes|process_(cpu_seconds_total|resident_memory_bytes)|neko_(capture_(streamsink_(bitrate|listeners|bytes)|pipelines_active)|media_(deliveries|delivery_(opens|closes)_total|source_generation|subscriptions_active|subscription_(delivered_(bytes|units)_total|discontinuities_total|dropped_units_total|queue_(capacity|depth))|websocket_(connections|handshakes_total|records_total|bytes_total|drops_total|resyncs_total|write_duration_seconds|queue_depth|queue_bytes|client_lag_milliseconds))|webrtc_(connection_state|receiver_estimated_target_bitrate|track_dropped_samples_total|video_listeners)))'

usage() {
  cat <<'EOF'
Collect credential-safe evidence for grouped WebCodecs/media-WebSocket validation.

Run from the repository root on the real target server:

  ./deploy/collect-webcodecs-media.sh init OUTPUT_DIR
  ./deploy/collect-webcodecs-media.sh snapshot OUTPUT_DIR PHASE
  ./deploy/collect-webcodecs-media.sh logs OUTPUT_DIR [SINCE]

Examples:

  ./deploy/collect-webcodecs-media.sh init ../neko-webcodecs-results
  ./deploy/collect-webcodecs-media.sh snapshot ../neko-webcodecs-results baseline-start
  ./deploy/collect-webcodecs-media.sh logs ../neko-webcodecs-results 60m

The collector inspects the base, adaptive-quality and WebCodecs overlay stack.
Set NEKO_METRICS_URL when metrics are not reachable at
http://127.0.0.1:8082/metrics. Compose may consume the ignored .env for normal
interpolation, but the script never prints or archives it, login credentials,
view-only tokens, media tickets or WebSocket URLs.
EOF
}
...
require_repository_root() {
  ...
  [[ -f "$COMPOSE_WEBCODECS" ]] || fail "missing $COMPOSE_WEBCODECS"
...
compose() {
  docker compose -f "$COMPOSE_BASE" -f "$COMPOSE_ADAPTIVE" -f "$COMPOSE_WEBCODECS" "$@"
}
