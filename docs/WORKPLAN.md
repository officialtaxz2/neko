...
Status: **repository assets implemented and statically reviewed on `testing` on 2026-09-14 / exact client checks/build, images and deployment pass through `f212dafd`, while default-off invariance passed earlier at `b6a0a5db` / f212 exposed duplicate client/server skew recovery plus terminal strict feedback timing, so their state-machine correction and the remaining matrix are NEXT/in progress / no prototype acceptance claimed**.
