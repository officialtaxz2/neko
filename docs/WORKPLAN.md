...
This repository path is not deployment or browser evidence. The server flag remains false by default; Phase 4 now supplies a separate explicit overlay, but no WebCodecs/media-WebSocket acceptance criterion is claimed as passed until the grouped target-server matrix completes.

## IN PROGRESS — WebCodecs/media-WebSocket Phase 4 deployment, observability and grouped validation

Status: **repository assets implemented and statically reviewed on `testing` on 2026-09-14 / automated, image, enabled deployment/security, continuous-stream, fullscreen and much of the functional matrix pass through exact `150aac57` / a visible foreground-iPhone audio-underflow cascade reached `resync_limit`; local-rebuffer correction `95746173` awaits exact target verification / presentation-pacing and the remaining latency/isolation/resource/hostile-input matrix are NEXT/in progress / no prototype acceptance claimed**.
...
The same `150aac57` matrix confirmed ordinary, admin and view-only VP8/Opus receive, private-mode pause/resume without reload, and explicit return to working WebRTC audio/video/control. A metrics interval added 14,393 VP8 units over 573 seconds (25.12/s), confirming the shared adaptive 25-fps cadence rather than a WebCodecs-only 25-versus-30 difference. It also isolated one short black/audio interruption to a common `client_audio_underflow` recovery. Later, a still-visible foreground iPhone sent three such recoveries in quick succession, reached the bounded `resync_limit` close and automatically retried; this is a Phase 4 failure, not background throttling. Commit `95746173` keeps 100-ms AudioWorklet starvation local, discards/accounted raced PCM, reanchors the next audio at 160-ms lead within the existing 200-ms cap and retains the canvas. Server rendered-lag/skew, decoder and overflow recovery remain unchanged. The correction was statically reviewed but **NOT EXECUTED IN CODEX**; exact client checks/image deployment and a clean foreground-iPhone interval are required before continuing the grouped matrix.

The exact URL selector and persistent in-video status remain deliberate prototype surfaces. After Phase 4 acceptance, the operator prefers a persisted per-client backend selector in the sidebar settings and a compact current-backend indicator, with the large overlay collapsed during healthy streaming. This is recorded as a later productization candidate; it does not replace the exact-query rollout boundary or expand the current receive-only prototype.
...
Execute and record **Phase 4 of the default-off WebCodecs plus dedicated media-WebSocket receive prototype** at the exact `testing` commit through [`WEBCODECS_MEDIA_WEBSOCKET_VALIDATION.md`](WEBCODECS_MEDIA_WEBSOCKET_VALIDATION.md). Phases 1–3 provide the protocol/ticket, server-delivery and isolated client receive boundaries; Phase 4's separate deployment/observability assets are now implemented and the operator-directed grouped target-server/browser acceptance is the remaining work.

Run and record the exact commit, images, overlays, browser/device versions and the full default-invariance, role/authorization/private-mode, startup/latency/A/V, slow-client/reconnect, cleanup/resource and hostile-input matrix. Use the fixed safe HTTP probe, evidence collector, PromQL and result template; do not expose credentials/tickets and do not claim acceptance from repository inspection alone.
...
2. **Phases 1–3 and Phase 4 repository assets implemented / grouped validation next:** perform exact-commit acceptance for the opt-in WebCodecs plus dedicated media WebSocket receive path;
3. prototype HLS/LL-HLS separately for passive/view-only device compatibility;
4. compare measured backends, then productize explicit per-client selection in sidebar settings with a compact backend/status indicator before considering automatic fallback;
...
7. **Phase 4 assets implemented / validation NEXT:** validate the completed prototype on the target server, including slow-client isolation and the documented malformed-input/resource matrix;
...
10. evaluate WebTransport only afterward if WebSocket's delivery/backpressure characteristics are a demonstrated limitation.
