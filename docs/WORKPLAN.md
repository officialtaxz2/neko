...
- The final opt-in candidate lowers `medium` to 748,800 bit/s and `low` to 332,800 bit/s, permits `max-quantizer: 63`, and adjusts `upgrade_diff_threshold` to 1.75 for the new maximum adjacent ratio of about 2.67:1. It changes only the mounted profile, requires no image rebuild, and is the final tuning iteration before explicit acceptance or rejection.

Pending before acceptance: deploy the final config-only bitrate candidate, rerun the affected 1.3 Mbit/s and 0.7 Mbit/s phases plus final recovery, and capture resources/logs. If it passes, complete refresh/rejoin plus transient-interruption checks; otherwise reject the profile from the recorded evidence. The stable base Compose and default estimator behavior remain unchanged.
...
Pull the final config-only tuning candidate on the real target server, recreate the existing adaptive Compose deployment without rebuilding its already validated image, and repeat the affected constrained phases plus final recovery from [`ADAPTIVE_QUALITY.md`](ADAPTIVE_QUALITY.md). Archive the four-phase metric/result table, relevant estimator logs, host resource measurements, commit and image ID.
