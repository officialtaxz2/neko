...
Status: **repository assets implemented and statically reviewed on `testing` on 2026-09-14 / server tests, fuzz, build, images, default-off/on startup and the pre-upgrade security matrix passed through `b6a0a5db` / the browser defect follow-up and remaining browser/isolation/resource matrix are NEXT/in progress / no prototype acceptance claimed**.
...
The first supported-browser media run at `b6a0a5db` did not pass. Three successful attachments delivered 1,166,277 video bytes, 441 audio bytes, 65 VP8 UNITs and 147 Opus UNITs, then each delivery failed after two accepted `client_queue_overflow` resyncs and the bounded third-resync rejection. Capture remained healthy and every subscription/delivery/socket cleaned up. Static tracing identified a browser accounting race: `VideoDecoder.decodeQueueSize` may decrease before its output callback runs, so the worker could admit another decode while two decoded/render slots were already committed and mistake a normal decoder-output burst for overflow. The follow-up reserves each video render slot until its output callback, retains the two-frame bound, and still resyncs a genuine bound violation. It also keeps the socket close handler attached after `END backend_error`, allowing the following private 4413/4500 close code to drive the already specified bounded same-backend retry instead of prematurely presenting a generic terminal error. A fixed-enum server close log now exposes the local close code/reason without credentials or payloads. These follow-up changes are statically reviewed only and require the complete exact-commit client/server checks, rebuilt images and a fresh browser run; Phase 4 remains open.
