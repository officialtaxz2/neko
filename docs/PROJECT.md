...
Automatic selection is a TARGET direction, not currently IMPLEMENTED. The immediate productization target is still manual and explicit: a per-client persisted `WebRTC`/`WebCodecs` choice in the existing sidebar settings, with WebRTC as the unset/default value and the URL parameter retained as a stateless diagnostic override. The active backend should use a compact settings/status indicator; the large prototype overlay should collapse during healthy streaming and remain prominent only for negotiation, recovery or terminal action. The operator selected this as the next implementation block after closing the bounded Phase 4 checkpoint; it is not implemented yet and does not authorize automatic fallback.

The repository implements the WebCodecs/media-WebSocket receive path through Phase 3 and the separate Phase 4 deployment/observability/validation assets. The server feature remains absent unless `docker-compose.webcodecs-ws.yaml` is explicitly included, the current client still requires exactly `?media=webcodecs-ws`, and an ordinary client sends no prototype requests or starts no prototype worker. The bounded exact `6b6cd328` target checkpoint passed automated, build, deployment/security, accumulated functional and corrected foreground-iPhone evidence, but the controlled presentation-pacing A/B and wider numeric latency/isolation/resource/live-hostile-input matrix remain explicitly unexecuted. Default WebRTC behavior remains unchanged, and the result is not represented as full prototype acceptance.
