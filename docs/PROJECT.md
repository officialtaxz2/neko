...
The repository now implements the WebCodecs/media-WebSocket receive path through Phase 3 and the separate Phase 4 deployment/observability/validation assets, but it is not yet a validated alternative. The server feature remains absent unless `docker-compose.webcodecs-ws.yaml` is explicitly included, the client requires exactly `?media=webcodecs-ws`, and an ordinary client sends no prototype requests or starts no prototype worker. Phase 4 automated checks and the deployment/security boundary pass at `b68f1fbf`; its stable-but-black browser result exposed renderer callbacks bound to vue-class-component's synthetic data instance, whose prototype-method correction still requires exact-commit automated and browser evidence. Default WebRTC behavior remains unchanged.
