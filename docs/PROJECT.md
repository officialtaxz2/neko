...
The exact first `webcodecs-ws` receive-prototype contract is fixed in [`WEBCODECS_MEDIA_WEBSOCKET.md`](WEBCODECS_MEDIA_WEBSOCKET.md). Its Phase 1 protocol/ticket/negotiation primitives, Phase 2 credential-free server delivery endpoint and Phase 3 isolated browser receive path are implemented. The endpoint owns strict pre-upgrade policy, bounded provider/egress/lifecycle queues, exact VP8/Opus serialization, READY/progress/resync state, private-mode and central delivery cleanup, and credential-safe metrics. Only the exact client query selects a dedicated worker with strict parsing, WebCodecs decoders, bounded AudioWorklet/canvas presentation and explicit recovery/fallback actions; without that query, or while the server flag is false, WebRTC remains unchanged. Phase 4 repository assets add a separate sanitized overlay, fixed-metric PromQL/evidence collection and an interactive target-server runbook without altering base Compose. At exact `86893473`, all 17 client tests, TypeScript/build, the complete server package sequence, 790,415 fuzz executions, trailing build, fresh images, enabled deployment and security probes passed. A continuous technical browser interval then added 46,267 Opus and 23,136 VP8 units over roughly 15 minutes with one active delivery, no resync metric, no failed WebCodecs close and no restart. The operator observed a continuously visible picture without conspicuous black flashes or stutters and audible approximately synchronized audio. Its approximately 25 VP8 units/s are the configured cadence of the shared adaptive `high` source, not evidence of WebCodecs transport loss; the canvas path nevertheless appeared subjectively slightly less fluid than WebRTC. Commit `3c780664` statically corrects the subsequently reported mobile fullscreen failure by continuing after rejected native requests, preserving iPhone's video-element fullscreen path and providing an explicit viewport-filling fallback when a WebCodecs canvas cannot enter native fullscreen. Target-server build/device evidence for that correction, a controlled presentation-pacing A/B and the remaining grouped matrix are open. Target-browser acceptance is therefore incomplete. HLS/LL-HLS, automatic backend selection and WebTransport remain later candidates.
...
Automatic selection is a TARGET direction, not currently IMPLEMENTED. Manual/explicit selection may be used first for diagnosis and rollout. After prototype acceptance, the preferred productization is a per-client persisted `WebRTC`/`WebCodecs` choice in settings, with the URL parameter retained only as a diagnostic override. The active backend should use a compact settings/status indicator; the large prototype overlay should collapse during healthy streaming and remain prominent only for negotiation, recovery or terminal action. This is a follow-up candidate, not current Phase 4 scope or implemented behavior.

The repository now implements the WebCodecs/media-WebSocket receive path through Phase 3 and the separate Phase 4 deployment/observability/validation assets, but it is not yet a validated alternative. The server feature remains absent unless `docker-compose.webcodecs-ws.yaml` is explicitly included, the client requires exactly `?media=webcodecs-ws`, and an ordinary client sends no prototype requests or starts no prototype worker. The renderer, underflow and feedback/skew/resync corrections pass their exact automated and initial technical streaming checkpoints at `86893473`; the operator also confirmed stable visible video and approximately synchronized audible audio in that continuous interval. A controlled WebCodecs/WebRTC presentation-pacing A/B and the wider role/latency/isolation/resource/hostile-input matrix remain open. Default WebRTC behavior remains unchanged.
