...
Automatic selection is a TARGET direction, not currently IMPLEMENTED. Manual selection is implemented in the repository: the existing sidebar persists a per-client `WebRTC`/`WebCodecs` default, WebRTC is the absent/invalid value, and the exact URL parameter remains a stateless highest-priority diagnostic override. The active backend is reported in settings; healthy WebCodecs streaming uses a compact in-player indicator, while the large status and explicit actions remain for negotiation, recovery or terminal failure. The focused exact-commit target-server checkpoint passed at `12cfe43b` with the live view-only fragment case explicitly omitted; this does not authorize automatic fallback or broaden the earlier prototype-acceptance claim.
