...
Status: **design and Phases 1–4 repository work implemented on `testing`; bounded target checkpoint closed at exact documentation commit `6b6cd328` after exact automated/build/security, accumulated functional and corrected foreground-iPhone gates passed; persisted manual per-client selection and compact healthy status passed their focused target checkpoint at `12cfe43b` with live view-only fragment preservation explicitly omitted; numeric latency/pacing, induced isolation, resource and remaining live hostile-input cases were explicitly deferred, so no full prototype-acceptance or automatic-fallback claim is made**.
...
On 2026-09-19, exact productization commit `12cfe43b` passed all 26 client tests, TypeScript and Vite build on the target server. Fresh base and Brave images built; Brave image `sha256:2295d7198e020a9ebf365c610188d15cf3e5c3950ceffa41d81d9d5a33b8a71b` deployed healthy with zero restarts and passed every recorded public/direct pre-upgrade probe. The operator confirmed safe missing/invalid storage, stored WebCodecs without a query across reload, return to WebRTC, exact-query precedence without saved-state mutation, unrelated-query preservation and compact healthy status. With the server overlay deliberately omitted, the client remained in the prominent terminal state `The server did not advertise the opt-in backend`, did not start WebRTC automatically and returned to working WebRTC only through the explicit action. Live view-only fragment preservation was not repeated; the focused automated navigation test passed. This closes only the client productization checkpoint and does not reopen or convert the wider deferred Phase 4 matrix into a pass.
...
Phase 1 adds the strict server envelope/fixtures, capture-side `PTSValid` propagation, single-use ticket storage, authenticated current/legacy negotiation events and the default-off server flag. Phase 2 adds the conditionally registered secure media route, credential-free provider-backed delivery, bounded queues/control/lifecycle recovery and fixed metrics. Phase 3 adds the initial exact-query client selection, shared-fixture parser, dedicated socket/decoder worker, AudioWorklet/canvas presentation and explicit recovery UI. Phase 4 owns deployment/observability assets and grouped acceptance; a later client-only productization adds persisted manual selection and compact healthy status without changing the transport contract.
...
The implementation has two independent opt-ins, with these names and defaults:

- server configuration `media.webcodecs_ws.enabled`, default `false`;
- client preference `media_backend=webcodecs-ws` in browser-local storage, default `webrtc`.
...
One continuously empty AudioWorklet queue for 100 ms is treated first as client-local scheduling starvation. The worklet discards and reports any raced queued PCM, the controller retains the last canvas image, reanchors the next audio PTS with 160 ms of lead and keeps both limits inside the existing 200-ms PCM cap. This does not request or consume a server resync. If audio presentation remains behind, the server's rendered-PTS lag and sustained-skew checks still initiate the bounded common recovery.
...
- sustained audio lag or audio overflow;
- decoder error;
...
- A normalized stored `media_backend=webcodecs-ws` preference selects the client path when the URL contains no `media` parameter. Exactly one `media=webcodecs-ws` query remains the stateless highest-priority diagnostic selection; wrong or duplicated query values use WebRTC. The legacy authenticated event socket then suppresses only its automatic WebRTC signal request and returns its current authenticated session ID in `system/init`; without an effective WebCodecs selection the original signal request and init wire shape remain unchanged.
...
- The main-thread controller selects exact advertised sources, requests a fresh ticket for every serialized attempt and owns the 80-ms initial clock anchors. A bounded stereo AudioWorklet is master when its 48-kHz context is running; otherwise video uses `performance.now()`. A single empty 128-frame render quantum starts but does not immediately publish an underflow; a new chunk cancels that observation. After 100 ms of continuous starvation, the client now retains the canvas and locally reanchors the next PCM at 160-ms lead inside the existing 200-ms cap instead of spending a common server-resync slot. Raced PCM is reported as discarded so worker accounting remains bounded. Server rendered-lag/skew enforcement remains the authority for sustained failure. Video-only server generations retain a valid normalized audio master, while actual audio/common server discontinuities clear both paths. The canvas renderer closes every rendered/dropped frame, drops video over 80 ms late and holds early frames for at most 100 ms. Resync refreshes feedback, rendered-progress and skew observation state, except that a progress-timeout resync retains its explicit two-second recovery deadline.
- Central Play resumes audio and starts rendering, mute/volume stay in the audio graph, and resize/fullscreen geometry uses the selected canvas without changing the existing video element/touch geometry. Native fullscreen remains preferred; rejected container requests continue to a surface request, iPhone's video-only API is invoked inside the original gesture for WebRTC, and a WebCodecs canvas without native support uses an explicitly escapable viewport-filling app mode that does not claim to hide mobile browser chrome. The opt-in UI labels the path receive-only, hides input/control/microphone and Picture-in-Picture actions, and exposes terminal **Retry WebCodecs** and **Use WebRTC** actions. The later productization follow-up persists the manual default in sidebar settings, reports the effective backend/override there and collapses the large healthy panel to a compact localized indicator while keeping nonhealthy states prominent.
...
- The first target-browser run at `b6a0a5db` completed three authenticated attachments and received VP8/Opus records, but each delivery reached the resync limit after false `client_queue_overflow` reports. Follow-ups corrected retry close handling, decode/render coupling, Vue callback binding, eager underflow, duplicate skew recovery and feedback pacing. At `86893473`, the full automated/image/deployment/security sequence and a roughly 15-minute zero-resync browser interval passed; approximately 25 VP8 units/s matched the shared adaptive `high` source. Exact `150aac57` later passed 19 client tests, TypeScript/build, fresh-image deployment and every pre-upgrade probe. The operator confirmed desktop native fullscreen, iPhone app fullscreen with browser chrome, ordinary/admin/view-only receive, private-mode pause/resume and explicit return to working WebRTC A/V/control. During that matrix, one visible foreground iPhone accumulated three rapid `client_audio_underflow` common recoveries, reached `resync_limit` and retried; the operator had also noticed short black/audio interruptions. Commit `95746173` removes that transient starvation from the common-resync path, locally reanchors with 160-ms lead and retains the canvas as described above. At exact documentation commit `6b6cd328`, all 20 client tests, TypeScript/build, the complete server suite, 849,148 fuzz executions, trailing build, images, deployment and security probes passed. One corrected foreground-iPhone connection then carried 16,372 VP8 and 32,792 Opus units for more than ten continuous minutes with no WebCodecs close, retry, server `client_audio_underflow`, progress timeout or restart; the operator reported picture and audio remained okay. The operator closed this bounded checkpoint while explicitly deferring controlled pacing/numeric latency, induced isolation, resource and remaining live hostile-input cases.
- Focused repository tests cover exact server query selection, ordinary-versus-selected legacy init serialization, shared-fixture acceptance and malformed framing/metadata/keyframe rejection in the exact browser parser source, plus the reused bounded retry schedule/code allowlist. The productization follow-up adds persistence/default/invalid/storage-denial cases, URL precedence and duplicate/wrong selector safety, query/fragment-preserving navigation, selection wiring and compact healthy versus prominent recovery/terminal status behavior.
...
Status: **repository assets implemented; bounded exact-commit target-server/browser checkpoint closed on 2026-09-14 at `6b6cd328`; the recorded subset passed, the omitted matrix remains pending and no full prototype acceptance is claimed**.
...
4. **Explicitly deferred on the target server:** complete the remaining matrix below before calling the prototype fully validated or making it an automatic/default backend.
...
1. choose WebRTC in the client setting (or clear the `media_backend` browser-local value) and remove `?media=webcodecs-ws` from diagnostic links;
...
There is no database migration, volume change, capture-format mutation or automatic fallback state to undo. The only client state is the explicit browser-local default, which the sidebar can reset to WebRTC. A failed prototype therefore rolls back to the existing WebRTC path without changing the stable deployment contract.
