...
Status: **design complete; Phases 1–3 protocol/ticket/negotiation, server delivery and isolated client receive path plus Phase 4's separate opt-in deployment/observability assets are implemented on `testing`; target-server tests/build/security/default-off checks passed through `b6a0a5db`, whose first browser run exposed a client render-slot accounting defect; the follow-up is statically reviewed and awaits exact-commit validation; no automatic fallback is implemented**.
...
- The first target-browser run at `b6a0a5db` completed three authenticated attachments and received VP8/Opus records, but each delivery reached the resync limit after false `client_queue_overflow` reports. `VideoDecoder.decodeQueueSize` can fall before its output callback; the follow-up therefore reserves one of the two decoded/render slots from `decode()` through output transfer/release instead of treating that callback gap as free capacity. The same run also exposed that handling `END backend_error` as immediately terminal detached the socket before its following 4413/4500 close code; the worker now waits for that exact close code so the fixed retry policy remains authoritative. Both corrections retain the documented bounds and require a fresh target-browser run.
