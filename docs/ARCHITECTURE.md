...
A 2026-09-19 direct-UDP trace then isolated a different steady-state defect: a healthy peer with zero reported loss, NACKs and local video drops repeatedly switched `high -> medium -> high -> medium` because the neutral-stall path interpreted failure to exceed the measured video rate by 15% as congestion. The corrected per-peer policy first builds a current delivery reference from `max(measured video, nominal video) + measured active audio`, then adds the configured `transport_reserve`. `diff_threshold` is now the tolerated sustained deficit below that reference; it is not upgrade reserve. A downward trend or neutral stall can downgrade only while that material deficit persists through the unchanged windows. Upgrades independently require `upgrade_diff_threshold` above the next tier's nominal complete-delivery reference. The resulting deadband prevents immediate reversal without disabling adaptive selection or sharing state across peers. Static tests cover neutral healthy capacity, real shortage, recovery, hysteresis and unchanged startup/backoff boundaries; target-server execution remains pending.
