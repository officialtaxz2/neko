...
The current fork still relies on Neko's WebRTC server model by default. When `media.webcodecs_ws.enabled` is false, WebRTC remains the only registered delivery adapter. Enabling that experimental server flag now also registers the specified credential-free `webcodecs-ws` backend and exact `/api/media/ws` route, but no client path selects or consumes it yet. The Phase 1 framing/ticket/negotiation and Phase 2 server adapter are specified in [`WEBCODECS_MEDIA_WEBSOCKET.md`](WEBCODECS_MEDIA_WEBSOCKET.md).
...
The source-subscription and participant-delivery interface semantics are decided. Version-1 WebCodecs framing/negotiation and the server media endpoint/backend/queue state machines are implemented through Phase 2. The isolated client path, deployment enablement, grouped target-server validation and target-device evidence remain open. HLS packaging parameters, the broader device/codec matrix and automatic selection remain OPEN until their evidence-led prototype blocks.
