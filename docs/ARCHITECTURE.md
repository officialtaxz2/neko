...
browser client
    ...
    | exact opt-in: event WebSocket + dedicated media WebSocket
...
The current fork still relies on Neko's WebRTC server model by default. When `media.webcodecs_ws.enabled` is false, WebRTC remains the only registered delivery adapter. Enabling that experimental server flag also registers the credential-free `webcodecs-ws` backend and exact `/api/media/ws` route. A client selects it only with exactly one `?media=webcodecs-ws`: the authenticated legacy event socket then omits `signal/request`, exposes its already authenticated session ID through `system/init`, negotiates a one-time ticket and keeps normal session/chat events while the dedicated media socket is owned by the worker. An unselected client follows the original signaling path byte-for-byte at the protocol boundary. The complete Phase 1–3 contract is specified in [`WEBCODECS_MEDIA_WEBSOCKET.md`](WEBCODECS_MEDIA_WEBSOCKET.md).
