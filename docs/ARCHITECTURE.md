...
The current fork still relies on Neko's WebRTC server model. WebRTC is the only registered delivery adapter behind the new boundary. The first `webcodecs-ws` receive prototype is specified in [`WEBCODECS_MEDIA_WEBSOCKET.md`](WEBCODECS_MEDIA_WEBSOCKET.md); Phase 1 implements framing, tickets and authenticated negotiation behind a default-off flag, but no media endpoint, delivery backend or client path exists yet.
...
The source-subscription and participant-delivery interface semantics are decided. Version-1 WebCodecs framing and VP8/Opus negotiation are implemented as Phase 1 primitives; the media endpoint/backend, queue state machines, client path and target-device evidence remain open. HLS packaging parameters, the broader device/codec matrix and automatic selection remain OPEN until their evidence-led prototype blocks.
