...
The passive HLS/LL-HLS specialization is fixed separately in [`HLS_LL_HLS.md`](HLS_LL_HLS.md), but is not implemented. It keeps one central participant delivery and a short-lived per-viewer HTTP lease while sharing one bounded in-memory packager set across viewers. The first output contract is separate fMP4 H.264 High 3.1 video renditions plus one AAC-LC audio rendition; conventional and low-latency playlists share six-second parents, while LL-HLS adds one-second parts and blocking reload. A ten-second event-plane ticket bootstraps an independent path-scoped Secure/HttpOnly cookie, so neither login/share credentials nor a playback bearer appears in playlist URIs. Same-origin HTTPS, trusted-proxy handling, no shared cache/CDN, bounded retention, private-mode/revocation semantics and exact device/resource/isolation gates are part of the design. No route, packager, player or selection is present yet.
