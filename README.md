...
- The default-off `webcodecs-ws` server boundary now provides Phase 1 strict version-1 framing, fixtures, PTS-validity propagation, authenticated negotiation and hashed single-use tickets plus the Phase 2 credential-free `/api/media/ws` delivery backend. The route is registered only with the server opt-in and enforces exact Origin/subprotocol/ticket/session/`CanWatch` checks before upgrade, bounded provider/egress/lifecycle queues, READY/progress limits, generation/keyframe resynchronization, private-mode/replacement/revocation/shutdown cleanup and low-cardinality metrics.
...
The first opt-in WebCodecs plus dedicated media-WebSocket receive prototype has an exact version-1 design covering credential-free ticket attachment, `CanWatch` lifecycle, VP8/Opus framing, bounded queues, A/V clocking, security limits, reconnect, rollout and target-server acceptance. Its Phase 1 primitives and Phase 2 server delivery adapter are implemented behind a default-off flag. No client WebCodecs/AudioWorklet/render path or deployment overlay is implemented yet, so this is not an end-to-end usable alternative transport. Per the operator's direction, the accumulated Phase 1/2 target-server tests and build are deferred to later grouped prototype validation; they were not executed in Codex.

## NEXT

Continue exclusively on `testing`: implement Phase 3 of the default-off `webcodecs-ws` receive prototype exactly within [`docs/WEBCODECS_MEDIA_WEBSOCKET.md`](docs/WEBCODECS_MEDIA_WEBSOCKET.md): the isolated opt-in client worker, strict shared-fixture parser, WebCodecs decoders, AudioWorklet/common clock, bounded render/decode queues, explicit UI selection/fallback actions and same-backend recovery. Keep WebRTC, its signaling/data channels and the stable deployment unchanged by default; add no deployment overlay yet, automatic fallback, HLS/LL-HLS, WebTransport or replacement control transport. Accumulated target-server validation remains deferred until the later grouped prototype checkpoint. `master` remains pinned at the accepted stable baseline until the operator explicitly authorizes a later grouped promotion.
