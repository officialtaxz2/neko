...
The first opt-in WebCodecs plus dedicated media-WebSocket receive prototype has an exact version-1 design covering credential-free ticket attachment, `CanWatch` lifecycle, VP8/Opus framing, bounded queues, A/V clocking, security limits, reconnect, rollout and target-server acceptance. Its Phase 1 protocol/ticket boundary, Phase 2 server delivery adapter and Phase 3 isolated client decode/render path are implemented behind the default-off server flag and exact `?media=webcodecs-ws` selection. The repository now contains the complete receive path, but no deployment overlay or target-server/browser acceptance exists yet. Per the operator's direction, accumulated tests, builds and runtime validation are deferred to the grouped prototype checkpoint; they were **NOT EXECUTED IN CODEX**.

## NEXT

Continue exclusively on `testing`: implement Phase 4 of the default-off `webcodecs-ws` receive prototype exactly within [`docs/WEBCODECS_MEDIA_WEBSOCKET.md`](docs/WEBCODECS_MEDIA_WEBSOCKET.md): add a separate sanitized opt-in Compose overlay and credential-safe observability assets, then prepare and execute the exact-commit grouped target-server/browser acceptance matrix outside Codex. Keep WebRTC, its signaling/data channels and the stable base deployment unchanged by default; add no automatic fallback, HLS/LL-HLS, WebTransport or replacement control transport. `master` remains pinned at the accepted stable baseline until the operator explicitly authorizes a later grouped promotion.
