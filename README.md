...
Continue exclusively on `testing`: implement **HLS/LL-HLS Phase 1** from [`docs/HLS_LL_HLS.md`](docs/HLS_LL_HLS.md): default-off configuration, authenticated bootstrap/playback-lease foundations, deterministic playlist/object models and golden fixtures, without starting media packaging or adding a client player. WebRTC remains the default, WebCodecs remains explicit, no fallback is automatic, and `master` remains pinned until the operator authorizes a later grouped promotion.
