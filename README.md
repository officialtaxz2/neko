...
- The exact default-off HLS/Low-Latency HLS passive/view-only version-1 contract is fixed in [`docs/HLS_LL_HLS.md`](docs/HLS_LL_HLS.md): one shared bounded packager set over the existing encoded-provider boundary, H.264/AAC fMP4 output, six-second parents/one-second LL parts, short-lived per-viewer cookie leases without URL credentials, same-room authorization/revocation, memory/cache/security limits, implementation phases and an exact target-device/resource/isolation matrix. This is design only; no HLS route, packager or player exists yet.
...
A later healthy-client trace showed a distinct sustained-selection defect: an active direct-UDP WebRTC session with zero reported loss, NACKs and local video drops still cycled `high -> medium -> high -> medium`. Reload only hid the defect by recreating the peer and estimator. The repository correction now treats `diff_threshold` as a tolerated sustained deficit below the current complete delivery requirement, not as upgrade-style spare capacity. Measured/nominal video, measured audio and the configured transport reserve form that requirement; `upgrade_diff_threshold` remains a separate next-tier reserve. Focused policy tests are implemented, while the exact target-server build and healthy-plus-constrained regression remain pending.
...
Continue exclusively on `testing`: validate the adaptive downgrade correction on one exact target-server commit with one healthy and one independently constrained WebRTC viewer using the focused procedure in [`docs/ADAPTIVE_QUALITY.md`](docs/ADAPTIVE_QUALITY.md). After that gate passes or its evidence is explicitly dispositioned, implement **HLS/LL-HLS Phase 1** from [`docs/HLS_LL_HLS.md`](docs/HLS_LL_HLS.md). WebRTC remains the default, WebCodecs remains explicit, no fallback is automatic, and `master` remains pinned until the operator authorizes a later grouped promotion.
