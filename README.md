...
The first opt-in WebCodecs plus dedicated media-WebSocket receive prototype has an exact version-1 design covering credential-free ticket attachment, `CanWatch` lifecycle, VP8/Opus framing, bounded queues, A/V clocking, security limits, reconnect, rollout and target-server acceptance. Its Phase 1 protocol/ticket boundary, Phase 2 server delivery adapter and Phase 3 isolated client decode/render path are implemented behind the default-off server flag and exact `?media=webcodecs-ws` selection. Phase 4 repository assets provide the separate opt-in deployment and credential-safe observability/validation workflow. At exact `b68f1fbf`, all automated checks and deployment/security gates passed, but live diagnostics exposed renderer callbacks retaining vue-class-component-v7's synthetic data instance. The `f7a9de4e` prototype-method correction then passed all 15 client tests, TypeScript/build and deployed healthy; the operator confirmed visible 1280×720 video, audible approximately synchronized audio and stable streaming with one active delivery. Six immediate `client_audio_underflow` recoveries nevertheless matched repeated roughly one-second black flashes because each single empty audio quantum reset both A/V paths. The current follow-up requires 100 ms of sustained audio starvation before that existing common recovery fires. It is statically reviewed but still requires an exact target build and browser rerun, so prototype acceptance remains pending. Project code, tests, builds and runtime checks were **NOT EXECUTED IN CODEX**.
