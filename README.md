...
The first opt-in WebCodecs plus dedicated media-WebSocket receive prototype now has an exact version-1 design covering credential-free ticket attachment, `CanWatch` lifecycle, VP8/Opus framing, bounded queues, A/V clocking, security limits, reconnect, rollout and target-server acceptance. This is a design artifact only: no route, backend or client transport is implemented.

## NEXT

Continue exclusively on `testing`: implement the first default-off `webcodecs-ws` receive prototype exactly within [`docs/WEBCODECS_MEDIA_WEBSOCKET.md`](docs/WEBCODECS_MEDIA_WEBSOCKET.md). Keep WebRTC, its signaling/data channels and the stable deployment unchanged by default; add no automatic fallback, HLS/LL-HLS, WebTransport or replacement control transport. The implementation and its target-server validation remain separate reviewed blocks. `master` remains pinned at the accepted stable baseline until the operator explicitly authorizes a later grouped promotion.
