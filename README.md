...
Continue exclusively on `testing`: specify the exact opt-in WebCodecs plus dedicated media-WebSocket prototype contract before adding the transport. Fix the authenticated delivery handshake and authorization/revocation flow, versioned binary audio/video framing, codec/capability negotiation, bounded server/browser queues, drop-to-keyframe resynchronization, A/V timing, explicit selection and rollback, observability, security limits and target-server acceptance gates. WebRTC must remain the unchanged default, current control/data-channel behavior stays separate, and this design block must add no endpoint or client/backend implementation. HLS/LL-HLS remains a later independent passive/view-only prototype. `master` remains pinned at the accepted stable baseline until the operator explicitly authorizes a later grouped promotion.
