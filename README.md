...
The first opt-in WebCodecs plus dedicated media-WebSocket receive prototype has an exact version-1 design covering credential-free ticket attachment, `CanWatch` lifecycle, VP8/Opus framing, bounded queues, A/V clocking, security limits, reconnect, rollout and target-server acceptance. Its Phase 1 protocol/ticket boundary, Phase 2 server delivery adapter and Phase 3 isolated client decode/render path are implemented behind the default-off server flag and exact `?media=webcodecs-ws` selection. Phase 4 repository assets provide the separate opt-in deployment and credential-safe observability/validation workflow. At exact `b68f1fbf`, all 13 client tests, TypeScript, the production build, the complete server suite, 30-second fuzzing, the trailing server build, fresh images, enabled startup and the live security matrix passed. Its browser rerun kept one delivery active without the former reconnect loop and received 669 VP8 units/5.57 MB, but the canvas remained black. Live browser diagnostics then proved that valid 1280-wide frames reached the component with correct 80–112 ms scheduling while its real Vue generation stayed zero and its animation handle remained stale. The current follow-up converts the three WebCodecs renderer callbacks from vue-class-component-v7 synthetic-instance arrow fields into bound prototype methods and also releases frames when no renderer listener exists. It is statically reviewed but still requires an exact target build and browser rerun, so prototype acceptance remains pending. Project code, tests, builds and runtime checks were **NOT EXECUTED IN CODEX**.
