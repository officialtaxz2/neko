...
- The exact default-off HLS/Low-Latency HLS passive/view-only version-1 contract is fixed in [`docs/HLS_LL_HLS.md`](docs/HLS_LL_HLS.md). Phases 1–2 now implement validated configuration and negotiation, digest-only tickets and sliding path-scoped cookie leases, the shared one-audio/three-video bounded packager, H.264 High 3.1/AAC-LC fMP4 output, deterministic conventional/LL-HLS playlists, conditional authenticated HTTP delivery, central per-session lifecycle and fixed credential-safe metrics. There is still no HLS client player, deployment overlay, automatic selection or target-runtime acceptance evidence.
...
Continue exclusively on `testing`: implement **HLS/LL-HLS Phase 3** from [`docs/HLS_LL_HLS.md`](docs/HLS_LL_HLS.md): the isolated passive browser client with pinned local player support, explicit manual `HLS`/`Low-Latency HLS` selection only when advertised, credential-free navigation, native-Apple versus MSE capability handling and complete pause/revoke cleanup. Preserve WebRTC as the absent/invalid default, keep WebCodecs explicit, add no automatic fallback or deployment overlay yet, and leave `master` pinned until the operator authorizes a later grouped promotion.
