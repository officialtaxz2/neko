...
Continue exclusively on `testing`: implement **HLS/LL-HLS Phase 1** from [`docs/HLS_LL_HLS.md`](docs/HLS_LL_HLS.md). Add only the default-off validated configuration/capability models, authenticated negotiation messages, ten-second one-time bootstrap ticket, short-lived path-scoped playback-lease/cookie state, exact HTTP security/limit/redaction helpers and deterministic playlist/object/generation models with golden fixtures and focused tests. Do not register a usable media route, start a provider subscription/transcoder/packager, add a player/dependency/selection, or change base deployment behavior in this phase. WebRTC remains the default, WebCodecs remains explicit, no fallback is automatic, and `master` remains pinned until the operator authorizes a later grouped promotion.
