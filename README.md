...
The first opt-in WebCodecs plus dedicated media-WebSocket receive prototype has an exact version-1 design covering credential-free ticket attachment, `CanWatch` lifecycle, VP8/Opus framing, bounded queues, A/V clocking, security limits, reconnect, rollout and target-server acceptance. Its Phase 1 protocol/ticket boundary, Phase 2 server delivery adapter and Phase 3 isolated client decode/render path are implemented behind the default-off server flag and exact `?media=webcodecs-ws` selection. Phase 4 repository assets provide the separate opt-in deployment and credential-safe observability/validation workflow. Exact target-server client/server checks, fuzzing, builds, fresh images, healthy default-off/on startup and the live pre-upgrade security matrix passed through `c4a316d5`; its browser rerun still remained black and repeatedly reached the queue-resync limit. The revised follow-up keeps decoding VP8 while locally closing output beyond the two-frame renderer hand-off, distinguishes bounded audio/video overflow sources and resets a retry episode only after 30 stable seconds. It is statically reviewed but not yet rebuilt or browser-validated, so prototype acceptance remains pending. Project code, tests, builds and runtime checks were **NOT EXECUTED IN CODEX**.
