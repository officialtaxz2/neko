...
Continue exclusively on `testing`: specify the exact default-off **HLS/Low-Latency HLS passive/view-only prototype contract** without implementing a transport yet. Fix how passive authorization and revocation remain in the same logical room, how short-lived playlist/segment access avoids credential leakage, how one bounded packager per active variant consumes the existing encoded-media boundary, which codec/device matrix is required, and how segment/part retention, discontinuities, cache/proxy behavior, cleanup, observability, latency and resource/isolation gates are measured. WebRTC remains the default interactive path, the explicit WebCodecs path remains unchanged, and no automatic fallback, DASH/WebTransport implementation or new control transport is added in this design block. `master` remains pinned at the accepted stable baseline until the operator authorizes a later grouped promotion.
