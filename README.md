...
The backend-neutral media-subscription/WebRTC compatibility refactor is implemented and statically reviewed on `testing`. At exact commit `a32027d`, the expanded target-server Go suite and server/plugin build passed, the local base and Brave images built, the adaptive service started healthy, and the current-protocol media lifecycle plus the new credential-free `neko_media_*` metrics behaved as designed. The broader manual matrix is not yet closed.

During that checkpoint, fresh viewers switched from `high` to `medium` on the first neutral estimator reading. The same behavior reproduced with the pre-refactor rollback image, proving it was not introduced by the media boundary. Static tracing found an inherited zero-time defect in the estimator's initial unstable/stalled observation windows. The repository correction initializes those windows at estimator-reader startup without changing bitrate, FPS, threshold, duration, protocol or deployment settings. It still requires exact-commit target-server confirmation and does not claim to be the sole cause of every later quality decision.

## NEXT

Continue exclusively on `testing`: validate one exact commit containing both the no-new-transport compatibility refactor from [`docs/MEDIA_SUBSCRIPTION_BOUNDARY.md`](docs/MEDIA_SUBSCRIPTION_BOUNDARY.md) and the estimator startup-timing correction. Run the expanded media/capture/WebRTC/session checks and server/plugin build, rebuild the local image, then perform a fresh single-viewer adaptive startup trace before completing the ordinary/admin/view-only and full adaptive smoke matrix. A first neutral/low-headroom estimate must no longer downshift merely because an observation timestamp began at zero; a later downgrade under sustained insufficient capacity remains valid. WebCodecs/WebSocket and HLS remain later opt-in prototypes until this checkpoint is accepted. `master` remains pinned at the accepted stable baseline until the operator explicitly authorizes a later grouped promotion.
